...
exports.getCart = async(req,res)=>{
    try{
        ...
        let userCart = await Cart.findOne({ user: userId }).populate('items.product', 'name price');
        // .populate('items.product', 'name price');
        if(!userCart){return res.status(404).json({ message: `User doesn't have a cart, start adding items to your cart now!` })}
        //populate cart 
...
exports.removeItem = async(req,res)=>{
    try{
        ...
        let userCart = await Cart.findOne({ user: userId });
        if(!userCart){return res.status(404).json({ message: `User doesn't have a cart, start adding items to your cart now!` })}
        //user have a cart=> now search for item and delete it
        const itemExists = userCart.items.find((item) => item.product.toString() === productId);
        if(!itemExists){return res.status(404).json({ message: `Product isn't in your cart!` })}
        //return new items array without product
        userCart.items = userCart.items.filter((item)=>{
            item.product.isremoved= true;
        ...
        userCart.totalItemsPrice = total;
        await userCart.save();
        // { message:'Item cleared from cart successfully',
        return res.status(200).json( userCart )
    }
    catch(err){
        return res.status(500).json({ message: err.message });
